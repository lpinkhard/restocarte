...
export default function MenuItemOptionUpdateForm(props) {
  const {
    id: idProp,
    menuItemOption: menuItemOptionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = menuItemOptionRecord
      ? { ...initialValues, ...menuItemOptionRecord }
      : initialValues;
    setTitle(cleanValues.title);
    setPriceDelta(cleanValues.priceDelta);
    setExclusive(cleanValues.exclusive);
    setOrder(cleanValues.order);
    setOwner(cleanValues.owner);
    setErrors({});
  };
  const [menuItemOptionRecord, setMenuItemOptionRecord] = React.useState(
    menuItemOptionModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(MenuItemOption, idProp)
        : menuItemOptionModelProp;
      setMenuItemOptionRecord(record);
    };
    queryData();
  }, [idProp, menuItemOptionModelProp]);
  React.useEffect(resetStateValues, [menuItemOptionRecord]);
  const validations = {
    title: [{ type: "Required" }],
    priceDelta: [],
    exclusive: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            MenuItemOption.copyOf(menuItemOptionRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MenuItemOptionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || menuItemOptionModelProp)}
